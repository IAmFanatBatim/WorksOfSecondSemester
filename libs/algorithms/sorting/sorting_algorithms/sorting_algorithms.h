...
//Производит сортировку расческой массива array размера size.
//Перебираются пары элементов с шагом, изначально равным size - 1; если в этих парах порядок нарушен, совершается обмен
//Проход по массиву со сравнениями пар происходит с уменьшением шага в k раз, где k = 1.24733
//Продолжаясь до тех пор, пока шаг не станет равен 1 и на очередном проходе не будет совершено ни одного обмена
void combSort(int *array, size_t size);

//Производит сортировку вставками массива array размера size
//Улучшенная версия сортировки вставками: алгоритм совершается несколько раз, но сравнение и сдвиг элементов происходит не между соседними элементами,
//а с некоторым шагом, изначально равным половине длины массива и с каждой итерацией уменьшающийся в 2 раза, пока не достигнет 1
void shellSort(int *array, size_t size);

//Возвращает число, описывающееся i-ым байтом числа (байты нумеруются с 0, с конца)
//Вспомогательная функция для цифровой сортировки
int getIByte_(int n, int i);

//Записывает по адресам min и max соотвественно минимальное и максимальное значение байта под индексом byte_ind среди чисел массива array размера size
//Вспомогательная функция для цифровой сортировки
void getMinMaxByte_(int *array, size_t size, int *min, int *max, int byte_ind);

//Производит цифровую сортировку массива array размера size (как для положительных, так и для отрицательных чисел)
